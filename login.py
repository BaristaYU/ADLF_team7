...
board = list() #게시판 db
mailbox = list(list("") for i in range(10)) #메일함 db (최대 10개)
...
board.append(['0000',tag[0],'USB 혜화역 4시쯤','빨간 SANDISK','010-1234-1234',]) #테스트문구1
...
def initial(main,id):
    ...
    for index,i in enumerate(board):
        ...
        if is_user == 1: #유저면 완료버튼, 아니면 쪽지버튼
            comp_btn[index] = Button(main,text="완료",command=lambda: comp(index)) 
...
def mailscreen(id):
    ...
    listNodes = Listbox(frame, width=38, height=15, font=("맑은 고딕", 12))
    ...
    def mailupdate():
        ...
        listNodes.delete(0,END) #쪽지함 초기화
        ...
        if len(mails) > 0:
            for index, j in enumerate(mails):
                listNodes.insert(index, j[0]+":"+"\t"+j[1])

        listNodes.after(500,mailupdate)
...
def write():
    ...
    def lost():
        is_lost = 1
        is_find = 0
        weetak_lab.config(state=status[is_find])
        reward_lab.config(state=status[is_lost])
        reward_ent.config(state=status[is_lost])
        place1_btn.config(state=status[is_find])
        place2_btn.config(state=status[is_find])
        place3_btn.config(state=status[is_find])
    ...
    weetak_lab = Label(post,text="* 위탁 시 위탁장소를 세부사항에 적어주세요.",state=DISABLED)
    weetak_lab.place(x=150,y=130)
    ...
    place1_btn = Radiobutton(post,text='원위치',value=1,variable=placevar,font=('맑은 고딕', 12),state=DISABLED)
    place1_btn.place(x=150,y=270)
    place1_btn.select()
    place2_btn = Radiobutton(post,text='위탁',value=2,variable=placevar,font=('맑은 고딕', 12),state=DISABLED)
    place2_btn.place(x=250,y=270)
    place3_btn = Radiobutton(post,text='취득',value=3,variable=placevar,font=('맑은 고딕', 12),state=DISABLED)
    place3_btn.place(x=350,y=270)
