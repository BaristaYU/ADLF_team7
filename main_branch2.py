...
def mainscreen(id,name):
    ...
    write_btn = Button(main,text="글쓰기",command = lambda: write(main,id,mains),font=("나눔고딕", 12))
...
def initial(main,id,init):
    global boardnum, mainboard, info_btn, comp_btn, mail_btn, mains
    pos_y = 150
    ...
    for index,i in enumerate(board):
        ...
        if is_user == 1: #유저면 완료버튼, 아니면 쪽지버튼
            ...
            mail_btn[index] = "N/A"
            print(comp_btn[index])
            print(mail_btn[index])
        else:
...
def write(main,id,mains):
    ...
    def getdata(id,tagvar,placevar,mains):
    ...
    write_btn = Button(post, text="작성", command=lambda: getdata(id,tagvar.get(), placevar.get(),mains),font=('나눔고딕', 10))
